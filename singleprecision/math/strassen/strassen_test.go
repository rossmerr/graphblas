...
func TestMatrix_Multiply(t *testing.T) {
	...
	matrix := singleprecision.NewDenseMatrix(4, 4)
	matrix.Set(0, 0, 1)
	matrix.Set(0, 1, 2)
	matrix.Set(0, 2, 3)
	matrix.Set(0, 3, 4)
	...
	want := singleprecision.NewDenseMatrix(4, 4)
	want.Set(0, 0, 10)
	want.Set(0, 1, 20)
	want.Set(0, 2, 30)
	want.Set(0, 3, 40)
	...
	tests := []struct {
		...
		{
			name: "DenseMatrix",
			s:    singleprecision.NewDenseMatrix(4, 4),
		},
		{
			name: "CSCMatrix",
			s:    singleprecision.NewCSCMatrix(4, 4),
		},
		{
			name: "CSRMatrix",
			s:    singleprecision.NewCSRMatrix(4, 4),
		},
	}
