...
func Multiply(ctx context.Context, a, b f32.Matrix) f32.Matrix {
...
func MultiplyCrossoverPoint(ctx context.Context, a, b f32.Matrix, crossover int) f32.Matrix {
	if a.Columns() != b.Rows() {
	...
	n := b.Rows()
	if n <= crossover {
		matrix := f32.NewDenseMatrix(a.Rows(), b.Columns())
		f32.MatrixMatrixMultiply(ctx, a, b, nil, matrix)
		return matrix
	}

	size := n / 2

	a11 := f32.NewDenseMatrix(size, size)
	a12 := f32.NewDenseMatrix(size, size)
	a21 := f32.NewDenseMatrix(size, size)
	a22 := f32.NewDenseMatrix(size, size)

	b11 := f32.NewDenseMatrix(size, size)
	b12 := f32.NewDenseMatrix(size, size)
	b21 := f32.NewDenseMatrix(size, size)
	b22 := f32.NewDenseMatrix(size, size)
	...
	matrix := f32.NewDenseMatrix(c11.Rows()*2, c11.Rows()*2)
	shift := c11.Rows()
...
func subMatrixM(ctx context.Context, out chan *mPlace, m int, a, b f32.Matrix, crossover int) {
	out <- &mPlace{
		m:      m,
