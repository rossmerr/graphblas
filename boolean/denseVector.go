...
func NewDenseVector(l int) *DenseVector {
	return &DenseVector{l: l, values: make([]bool, l)}
}

// NewDenseVectorFromArray returns a SparseVector
func NewDenseVectorFromArray(data []bool) *DenseVector {
	arr := make([]bool, 0)
	arr = append(arr, data...)
...
func (s *DenseVector) AtVec(i int) bool {
	if i < 0 || i >= s.Length() {
...
func (s *DenseVector) SetVec(i int, value bool) {
	if i < 0 || i >= s.Length() {
		log.Panicf("Length '%+v' is invalid", i)
	}
...
func (s *DenseVector) Update(r, c int, f func(bool) bool) {
	if r < 0 || r >= s.Rows() {
...
func (s *DenseVector) At(r, c int) (value bool) {
	return s.AtVec(r)
}

// Set sets the value at r-th, c-th of the vector
func (s *DenseVector) Set(r, c int, value bool) {
	if r < 0 || r >= s.Rows() {
		log.Panicf("Row '%+v' is invalid", r)
	}
...
func (s *DenseVector) RowsAtToArray(r int) []bool {
	if r < 0 || r >= s.Rows() {
		log.Panicf("Row '%+v' is invalid", r)
	}
...
func (s *denseVectorIterator) Next() (int, int, bool) {
	s.next()
...
func (s *denseVectorMap) Map(f func(int, int, bool) bool) {
	s.next()
