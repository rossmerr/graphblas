...
func TestMatrix_FromArray(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "DenseMatrix",
			s:    f64.NewDenseMatrixFromArray(setup),
		},
		{
			name: "CSCMatrix",
			s:    f64.NewCSCMatrixFromArray(setup),
		},
		{
			name: "CSRMatrix",
			s:    f64.NewCSRMatrixFromArray(setup),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
