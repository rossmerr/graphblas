...
func TestVector_Update(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "DenseVector",
			s:     f64.NewDenseVector(2),
			want:  2,
			value: 2,
		},
		{
			name:  "SparseVector",
			s:     f64.NewSparseVector(2),
			want:  2,
			value: 2,
		},
...
func TestVector_Transpose(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "DenseVector",
			s:    f64.NewDenseVector(3),
		},
		{
			name: "SparseVector",
			s:    f64.NewSparseVector(3),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestVector_Multiply(t *testing.T) {
	...
	want := f64.NewDenseMatrix(2, 1)
	want.Set(0, 0, 27)
	want.Set(1, 0, 41)

	matrix := f64.NewDenseMatrix(2, 3)
	matrix.Set(0, 0, 0)
	matrix.Set(0, 1, 3)
	matrix.Set(0, 2, 5)
	matrix.Set(1, 0, 5)
...
func TestVector_Size(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "DenseVector",
			s:    f64.NewDenseVector(6),
			size: 6,
		},
		{
			name: "SparseVector",
			s:    f64.NewSparseVector(6),
			size: 5,
		},
	}
	for _, tt := range tests {
