...
func TestVector_Update(t *testing.T) {
	...
	setup := func(m doublePrecision.Vector) {
		m.SetVec(0, 3)
		m.SetVec(1, 8)
	}
	...
	tests := []struct {
		...
		{
			name:  "DenseVector",
			s:     doublePrecision.NewDenseVector(2),
			want:  2,
			value: 2,
		},
		{
			name:  "SparseVector",
			s:     doublePrecision.NewSparseVector(2),
			want:  2,
			value: 2,
		},
...
func TestVector_RowAt(t *testing.T) {
	...
	want := doublePrecision.NewDenseVector(1)
	want.SetVec(0, 4)
...
func TestVector_Transpose(t *testing.T) {
	...
	want := doublePrecision.NewDenseMatrix(1, 3)
	want.Set(0, 0, 6)
	want.Set(0, 1, 4)
	want.Set(0, 2, 24)
	...
	tests := []struct {
		...
		{
			name: "DenseVector",
			s:    doublePrecision.NewDenseVector(3),
		},
		{
			name: "SparseVector",
			s:    doublePrecision.NewSparseVector(3),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestVector_Multiply(t *testing.T) {
	...
	want := doublePrecision.NewDenseMatrix(2, 1)
	want.Set(0, 0, 27)
	want.Set(1, 0, 41)

	matrix := doublePrecision.NewDenseMatrix(2, 3)
	matrix.Set(0, 0, 0)
	matrix.Set(0, 1, 3)
	matrix.Set(0, 2, 5)
	matrix.Set(1, 0, 5)
...
func TestVector_Add(t *testing.T) {
	...
	matrix := doublePrecision.NewDenseVector(2)
	matrix.SetVec(0, 4)
	matrix.SetVec(1, 0)
...
func TestVector_Subtract(t *testing.T) {
	...
	matrix := doublePrecision.NewDenseMatrix(2, 1)
	matrix.Set(0, 0, 4)
	matrix.Set(1, 0, 0)
...
func TestVector_Size(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "DenseVector",
			s:    doublePrecision.NewDenseVector(6),
			size: 6,
		},
		{
			name: "SparseVector",
			s:    doublePrecision.NewSparseVector(6),
			size: 5,
		},
	}
	for _, tt := range tests {
